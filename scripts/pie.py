...
def generate_variant_map(df, output_dir):
    ...
    geo_merge = geo_ch.assign(
        region=lambda y: y.HASC_1.apply(lambda x: iso_canton_to_region[x[-2:]])
    )
    ylgn = cm.get_cmap("YlGnBu", 512)
    newcmp = ListedColormap(ylgn(np.linspace(0.25, 0.75, 256)))

    geo_merge.plot(column="region", figsize=(8, 5), cmap=newcmp)
    ...
    for reg in range(1, 7):
        ...
        if len(variant_to_lineage) == 5:
            colors = [
                (0.819607843137255, 0.4, 0.4),
                (1, 0.4, 0.4),
                (1, 0.701960784313725, 0.701960784313725),
                (0.4, 0.63921568627451, 0.4),
                (0.956862745098039, 0.862745098039216, 0.505882352941176),
                (0, 0, 0),
            ]
        ...
        draw_pie(ratios, colors, x_val, y_val, size=0.5)
    ...
    plt.title(
        f'Variants by Region between CW {df.index[0].strftime("%W-%Y")} and CW {df.index[-1].strftime("%W-%Y")}',
        size=15,
    )
