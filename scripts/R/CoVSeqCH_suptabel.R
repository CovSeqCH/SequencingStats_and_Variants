...
variants_ch <- subset(variants_ch, as_date(date) %in% seq(period_date[1],period_date[2],1))
...
for (c in unname(unlist(rownames(table)))){
  #population size:
  if(grepl("region", c)){
    table[,1][rownames(table) == c] <- sum(unique(na.omit(BAG_data$pop[BAG_data$region %in% c])))
  }
  else{
  ...
  #confirmed cases:
  if(grepl("region", c)){
    table[,2][rownames(table) == c] <- sum(unique(na.omit(BAG_data$cases_num[BAG_data$region %in% c])))
  }
  else{
    table[,2][rownames(table) == c] <- sum(unique(BAG_data$cases_num[BAG_data$geoRegion %in% c]))
  }
  #14d-incidence
  period_14 <- length(period_days)/14
  if(grepl("region", c)){
    table[,3][rownames(table) == c] <- round(sum(unique(na.omit(BAG_data$cases_num[BAG_data$region %in% c])))/sum(unique(na.omit(BAG_data$pop[BAG_data$region %in% c]))*period_14)*10^5)
  }
  else{
    table[,3][rownames(table) == c] <- round(sum(unique(BAG_data$cases_num[BAG_data$geoRegion %in% c]))/sum(unique(BAG_data$pop[BAG_data$geoRegion %in% c])*period_14)*10^5)
  }
  ...
  if(grepl("region", c)){
    table[,6][rownames(table) == c] <- round(sum(na.omit(BAG_data$tests_num[BAG_data$region %in% c]))/sum(unique(na.omit(BAG_data$pop[BAG_data$region %in% c])))*10^5)
  }
  else{
  ...
  # test positivity
  if(grepl("region", c)){
    table[,7][rownames(table) == c] <- format(round(sum(na.omit(BAG_data$tests_pos_num[BAG_data$region %in% c]))/sum(unique(na.omit(BAG_data$tests_num[BAG_data$region %in% c])))*10^2,2), nsmall = 2)
  }
  else{
    table[,7][rownames(table) == c] <- format(round(sum(na.omit(BAG_data$tests_pos_num[BAG_data$geoRegion %in% c]))/sum(unique(BAG_data$tests_num[BAG_data$geoRegion %in% c]))*10^2,2), nsmall = 2)
  }
  # number of sequenced samples 
  if(grepl("region|CH", c)){
    table[,8][rownames(table) == c] <- sum(variants_ch$value[variants_ch$region %in% c])
  }
  # Proportion of cases that have been sequenced
  if(grepl("region|CH", c)){
    table[,9][rownames(table) == c] <- format(round(sum(variants_ch$value[variants_ch$region %in% c])/sum(variants_ch$cases[variants_ch$region %in% c])*100,1), nsmall = 1)
  }
  # number of  "alpha" 
  if(grepl("region|CH", c)){
    table[,10][rownames(table) == c] <- sum(variants_ch$value[variants_ch$region %in% c & variants_ch$who_variants  %in% "Alpha"])
  }
  # Percentage of  "alpha" to all sequences
  if(table[,10][rownames(table) == c]!=0 & !is.na(table[,10][rownames(table) == c])){
    interval <- binom.test(as.numeric(table[,10][rownames(table) == c]), sum(variants_ch$value[variants_ch$region %in% c]))  # binomial 95% confidence intervals
  ...
  # Number of  beta 
  if(grepl("region|CH", c)){
    table[,12][rownames(table) == c] <- sum(variants_ch$value[variants_ch$region %in% c & variants_ch$who_variants  %in% "Beta"])
  }
  # Proportion of  beta to all sequences
  if(table[,12][rownames(table) == c]!=0 & !is.na(table[,12][rownames(table) == c])){
    interval <- binom.test(as.numeric(table[,12][rownames(table) == c]), sum(variants_ch$value[variants_ch$region %in% c]))  #binomial 95% confidence intervals
  ...
  # Number of gamma 
  if(grepl("region|CH", c)){
    table[,14][rownames(table) == c] <- sum(variants_ch$value[variants_ch$region %in% c & variants_ch$who_variants  %in% "Gamma"])
  }
  # Proportion of  Gamma to all sequences
  if(table[,14][rownames(table) == c]!=0 & !is.na(table[,14][rownames(table) == c])){
    interval <- binom.test(as.numeric(table[,14][rownames(table) == c]), sum(variants_ch$value[variants_ch$region %in% c]))  #binomial 95% confidence intervals
  ...
  # Number of delta 
  if(grepl("region|CH", c)){
    table[,16][rownames(table) == c] <- sum(variants_ch$value[variants_ch$region %in% c & variants_ch$who_variants  %in% "Delta"])
  }
  # Proportion of  Delta to all sequences
  if(table[,16][rownames(table) == c]!=0 & !is.na(table[,16][rownames(table) == c])){
    interval <- binom.test(as.numeric(table[,16][rownames(table) == c]), sum(variants_ch$value[variants_ch$region %in% c]))  #binomial 95% confidence intervals
  ...
  # Number of C.36* 
  if(grepl("region|CH", c)){
    table[,18][rownames(table) == c] <- sum(variants_ch$value[variants_ch$region %in% c & variants_ch$who_variants  %in% "C.36*"])
  }
  # Proportion of  C.36* to all sequences
  if(table[,18][rownames(table) == c]!=0 & !is.na(table[,18][rownames(table) == c])){
    interval <- binom.test(as.numeric(table[,18][rownames(table) == c]), sum(variants_ch$value[variants_ch$region %in% c]))  #binomial 95% confidence intervals
  ...
  # Number of "other"
  if(grepl("region|CH", c)){
    table[,20][rownames(table) == c] <- sum(variants_ch$value[variants_ch$region %in% c & variants_ch$who_variants  %in% "others"])
  }
  # Proportion of  C.36* to all sequences
  if(table[,20][rownames(table) == c]!=0 & !is.na(table[,20][rownames(table) == c])){
    interval <- binom.test(as.numeric(table[,20][rownames(table) == c]), sum(variants_ch$value[variants_ch$region %in% c]))  #binomial 95% confidence intervals
